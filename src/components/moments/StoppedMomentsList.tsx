...
import { Clock, Calendar, MapPin, StickyNote, CheckCircle, PlayCircle } from "lucide-react";
...
const StoppedMomentsList: React.FC<StoppedMomentsListProps> = ({ moments }) => {
  ...
  const { continueMoment } = useMoment();
  ...
  const handleContinueMoment = () => {
    if (selectedMoment) {
      continueMoment(selectedMoment.id);
      setShowDetails(false);
      toast({
        title: "Moment continued",
        description: `"${selectedMoment.title}" has been moved back to active moments`,
      });
    }
  };
    ...
    <div className="mt-12 border-t border-[#e8c28233] pt-8">
      ...
      {selectedMoment && (
        <MomentDetailsDialog
          ...
          onContinue={handleContinueMoment}
        />
      )}
    </div>
  );
